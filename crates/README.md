...
the key is generated by the official app during local BLE onboarding and stored
in its Realm database. Pass it with `--key-file`.

On rooted Android, `tools/android_oura_key_extract.py` can pull the Oura app's
Realm and export the key after the phone has successfully paired with the ring.
If the app only has account/cloud ring metadata and no local BLE pairing, there
is no phone-side key to extract yet; complete the official app pairing flow first.
On the local already-onboarded Ring 5, the official Android app reached scanning
but failed at Android BLE bonding, before it could create or store a Realm key.
...
events to the store. Ring 5 sync uses Android's newer `ExtGetEvent` path when
available; its `0x43` stream is length-prefixed bundled events that are expanded
back into the same normal event packets returned by legacy `GetEvent`. The
per-event **body** layouts come from the ring's native
